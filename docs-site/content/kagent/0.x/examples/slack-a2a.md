---
title: Integrating kagent with Slack
linkTitle: Slack and A2A
...
We're using the builtin tools here. However, you can also use `kmcp` to create your own MCPServer resource to use your own tools.

To access the agent through A2A, we'll need to port-forward the kagent controller service to our local machine:
...
Now restart the bot with `uv run main.py`.

In Slack, add the kagent app to your channel. Then, try the `/mykagent` command to chat with the agent in the channel. For example, you can ask the agent to list the pods in the cluster:
...
Let's see how we can do the reverse - sending messages to Slack from kagent. We'll use an existing [Slack MCP server](https://github.com/modelcontextprotocol/servers/blob/2025.5.12/src/slack/README.md) that will allow us to send messages to Slack channels.
...
Create a Kubernetes secret with the Slack credentials:
